# -*- coding: utf-8 -*-
"""
Created on Mon Sep 25 12:42:59 2017
...
#Tuples containing setting names, note that additional variables can be added into the training arguments and sample options list. For each new argument, add two to the commandlist[#] to the commandList function
                    ...
                    if currentArg != 'primetext' and currentArg == currentDictKey : #Run everything not primetext as a normal command
                        ...
                        for t in range(remainingArgs):
                            ...
                            for z in range(max_args):
                                try:
                                    nextArg = arguments[i+t]
                                    #In case the primetext includes words that are also default argument parameters, they won't trigger the program into thinking they are the next parameter
                                    if nextArg == '-temperature' or nextArg == '-seed' or nextArg == '-verbose' or nextArg == '-sample' or nextArg == '-length' or nextArg == '-model':
                                        print('Argument detected after primetext')
